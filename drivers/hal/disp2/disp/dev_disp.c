...
static int disp_clk_get_wrap(struct disp_drv_info *disp_drv)
{
...
#if defined(CONFIG_ARCH_SUN20IW2)
        hal_sprintf(id, "clk_mbus_de%d", i);
    ...
    for (i = 0; i < DISP_DEVICE_NUM; i++) {
        /* get clocks for dpss */
        hal_sprintf(id, "clk_bus_dpss_top%d", i);
    ...
    for (i = 0; i < DEVICE_LVDS_NUM; i++) {
        hal_sprintf(id, "rst_bus_lvds%d", i);
