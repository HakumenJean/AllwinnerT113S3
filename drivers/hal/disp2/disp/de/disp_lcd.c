...
#else
    disp_clk_t lvds_clk;
    disp_clk_t dsi_clk[CLK_DSI_NUM];
    disp_clk_t edp_clk;
    disp_clk_t clk_parent;
#endif
...
static s32 lcd_parse_panel_para(u32 disp, struct disp_panel_para *info)
{
    ...
    hal_sprintf(primary_key, "lcd%u", (unsigned int) disp);
...
static void lcd_get_sys_config(u32 disp, struct disp_lcd_cfg *lcd_cfg)
{
    ...
    hal_sprintf(sub_name, "lcd_bl_en_power");
    ret =
    ...
    for (i = 0; i < LCD_POWER_NUM; i++) {
        if (i == 0)
            hal_sprintf(sub_name, "lcd_fix_power");
        else
            hal_sprintf(sub_name, "lcd_fix_power%d", i);
    ...
    for (i = 0; i < LCD_POWER_NUM; i++) {
        if (i == 0)
            hal_sprintf(sub_name, "lcd_power");
        else
            hal_sprintf(sub_name, "lcd_power%d", i);
        lcd_cfg->lcd_power_used[i] = 0;
        ret =
            disp_sys_script_get_item(primary_key, sub_name,
    ...
    for (i = 0; i < LCD_GPIO_NUM; i++) {
        hal_sprintf(sub_name, "lcd_gpio_%d", i);
    ...
    for (i = 0; i < LCD_GPIO_REGU_NUM; i++) {
        hal_sprintf(sub_name, "lcd_gpio_power%d", i);
    ...
    for (i = 0; i < LCD_GPIO_REGU_NUM; i++) {
        if (i == 0)
            hal_sprintf(sub_name, "lcd_pin_power");
        else
            hal_sprintf(sub_name, "lcd_pin_power%d", i);
        ret =
            disp_sys_script_get_item(primary_key, sub_name,
...
/* backlight adjust */
    for (i = 0; i < 101; i++) {
        hal_sprintf(sub_name, "lcd_bl_%d_percent", i);
    ...
    hal_sprintf(sub_name, "lcd_backlight");
...
static s32 cal_real_frame_period(struct disp_device *lcd)
{
    ...
    if (lcdp->panel_info.lcd_if == LCD_IF_DSI)
        lcd->timings.dclk_rate_set =
            disp_sys_clk_get_rate(lcdp->clk_tcon_lcd) / clk_info.dsi_div;
#endif
    if (lcdp->panel_info.lcd_if != LCD_IF_DSI)
        lcd->timings.dclk_rate_set =
...
static s32 lcd_clk_config(struct disp_device *lcd)
{
    ...
    if (lcdp->panel_info.lcd_if == LCD_IF_DSI) {
        ...
        for (i = dsi_num * lcd->disp, j = 0; i < CLK_DSI_NUM;
             i += CLK_NUM_PER_DSI) {
            if (lcdp->dsi_clk[i]) {
                parent = (disp_clk_t)disp_sys_clk_get_parent(lcdp->dsi_clk[i]);
                disp_sys_clk_set_parent(lcdp->dsi_clk[i], parent);
                disp_sys_clk_set_rate(lcdp->dsi_clk[i], dsi_rate_set);
                DE_INF(
                    "disp_sys_clk_set_rate:dsi's %d th clk with %ld\n",
                    i, dsi_rate_set);
                j += CLK_NUM_PER_DSI;
        ...
        /*FIXME, dsi clk0 = dsi clk1(rate)*/
        /*disp_sys_disp_sys_clk_set_rate(lcdp->dsi_clk[1], dsi_rate_set);*/
    }
    dclk_rate_set = lcd_rate_set / clk_info.tcon_div;
    if ((pll_rate_set != pll_rate) || (lcd_rate_set != lcd_rate)
        || (dclk_rate_set != dclk_rate)) {
...
static s32 lcd_clk_enable(struct disp_device *lcd)
{
    ...
    if (hal_clock_is_enabled(lcdp->clk_tcon_lcd) != HAL_CLK_STATUS_ENABLED) {
        ret = disp_sys_clk_enable(lcdp->clk_tcon_lcd);
        if (ret != 0) {
            DE_WRN("fail enable lcd's clock!\n");
            goto exit;
    ...
    if (lcdp->panel_info.lcd_if == LCD_IF_LVDS) {
        if (lcdp->lvds_clk) {
            parent = (disp_clk_t)disp_sys_clk_get_parent(lcdp->lvds_clk);
            disp_sys_clk_set_parent(lcdp->lvds_clk, parent);
            ret = disp_sys_clk_enable(lcdp->lvds_clk);
        }
        if (ret != 0) {
            DE_WRN("fail enable lvds's clock!\n");
            goto exit;
        ...
        for (i = dsi_num * lcd->disp, j = 0; i < CLK_DSI_NUM; ++i) {
            if (lcdp->dsi_clk[i]) {
                ret = disp_sys_clk_enable(lcdp->dsi_clk[i]);
                if (ret != 0) {
                    DE_WRN("fail enable dsi's clock%d!\n",
                           i);
                    goto exit;
        ...
        parent = (disp_clk_t)disp_sys_clk_get_parent(lcdp->edp_clk);
        disp_sys_clk_set_parent(lcdp->edp_clk, parent);
        ret = disp_sys_clk_enable(lcdp->edp_clk);
        if (ret != 0) {
            DE_WRN("fail enable edp's clock!\n");
            goto exit;
        }
...
static s32 lcd_clk_disable(struct disp_device *lcd)
{
    ...
    if (lcdp->panel_info.lcd_if == LCD_IF_LVDS) {
        ...
        for (i = dsi_num * lcd->disp, j = 0; i < CLK_DSI_NUM; ++i) {
            if (lcdp->dsi_clk[i])
                disp_sys_clk_disable(lcdp->dsi_clk[i]);
            ++j;
            if (j == dsi_num)
                break;
        }
...
static s32 disp_lcd_pin_cfg(struct disp_device *lcd, u32 bon)
{
    ...
    hal_sprintf(dev_name, "lcd%u", (unsigned int) lcd->disp);
...
s32 disp_lcd_gpio_exit(struct disp_device *lcd)
{
    ...
    for (i = 0; i < LCD_GPIO_NUM; i++) {
        if (lcdp->lcd_cfg.gpio_hdl[i]) {
            ...
            memcpy(gpio_info, &(lcdp->lcd_cfg.lcd_gpio[i]),
                   sizeof(struct disp_gpio_set_t));
            gpio_info->mul_sel = GPIO_MUXSEL_DISABLED;
            lcdp->lcd_cfg.gpio_hdl[i] =
                disp_sys_gpio_request(gpio_info, 1);
            disp_sys_gpio_release(lcdp->lcd_cfg.gpio_hdl[i], 2);
            lcdp->lcd_cfg.gpio_hdl[i] = 0;
                ...
                u32 direction)
{
    ...
    hal_sprintf(gpio_name, "lcd_gpio_%u", (unsigned int) io_index);
    ...
    if (disp_lcd_is_used(lcd)) {
        ...
        if (lcdp->panel_info.lcd_pwm_used) {
            ...
            if (lcdp->panel_info.lcd_pwm_freq != 0) {
                ...
                DE_WRN("lcd%u.lcd_pwm_freq is ZERO\n",
                       lcd->disp);
                /* default 1khz */
...
s32 disp_init_lcd(struct disp_bsp_init_para *para)
{
    ...
    for (hwdev_index = 0; hwdev_index < num_devices; hwdev_index++) {
        ...
        hal_sprintf(lcd->name, "lcd%u", (unsigned int) disp);
        lcd->disp = disp;
#if defined(CONFIG_ARCH_SUN8IW17P1)
        value = 0;
        ...
        lcdp->clk_tcon_lcd = para->mclk[DISP_MOD_LCD0 + lcd->hwdev_index];
