...
s32 bsp_disp_init(struct disp_bsp_init_para *para)
{
    ...
    disp_lock = (hal_spinlock_t)HAL_SPINLOCK_INIT;
    hal_spin_lock_init(&disp_lock);

    hal_memset(&gdisp, 0x00, sizeof(struct disp_dev_t));
    hal_memcpy(&gdisp.init_para, para, sizeof(struct disp_bsp_init_para));
...
s32 bsp_disp_get_screen_physical_width(u32 disp)
{
    ...
    if (mgr && mgr->device && mgr->device->get_dimensions)
        mgr->device->get_dimensions(mgr->device, (u32 *)&width, (u32 *)&height);
