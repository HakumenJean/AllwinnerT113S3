        ...
        struct scaler_para *ovl_cpara)
{
    ...
    /* init para */
    if ((VI_CHN_NUM != 0) && (LAYER_MAX_NUM_PER_CHN != 0)) {
        for (j = 0; j < vi_chn; j++)
            memset((void *)cpara[j], 0x0,
                layno * sizeof(struct scaler_para));
    };

    if ((CHN_NUM != 0) && (LAYER_MAX_NUM_PER_CHN != 0)) {
        for (j = 0; j < chn; j++)
            ...
            unsigned int layer_num, bool direct_show)
{
    ...
    for (kcon = 0; kcon < layer_num; kcon++) {
        ...
        count = 0;
        count += hal_sprintf(buf + count, " %5s ",
            (data_print.config.info.mode == LAYER_MODE_BUFFER) ?
            "BUF" : "COLOR");
        count += hal_sprintf(buf + count, " %8s ",
            (data_print.config.enable == 1) ? "enable" : "disable");
        count += hal_sprintf(buf + count, "ch[%1u] ",
                        data_print.config.channel);
        count += hal_sprintf(buf + count, "lyr[%1u] ",
                        data_print.config.layer_id);
        count += hal_sprintf(buf + count, "z[%1u] ",
                        data_print.config.info.zorder);
        count += hal_sprintf(buf + count, "prem[%1s] ",
            (data_print.config.info.fb.pre_multiply) ? "Y" : "N");
        count += hal_sprintf(buf + count, "a[%5s %3u] ",
        ...
        count += hal_sprintf(buf + count, "fb[%4u,%4u;%4u,%4u;%4u,%4u] ",
        ...
        count += hal_sprintf(buf + count, "crop[%4u,%4u,%4u,%4u] ",
        ...
        count += hal_sprintf(buf + count, "frame[%4d,%4d,%4u,%4u] ",
        ...
        count += hal_sprintf(buf + count, "addr[%8llx,%8llx,%8llx] ",
                    data_print.config.info.fb.addr[0],
                    data_print.config.info.fb.addr[1],
                    data_print.config.info.fb.addr[2]);
        count += hal_sprintf(buf + count, "flags[0x%8x] trd[%1d,%1d]\n",
