...
#define TVE_COUNT   1
static hal_spinlock_t g_tv_data_lock;
...
static s32 tv_clk_init(struct disp_device *ptv)
{
    ...
    ptvp->clk_parent = hal_clk_get_parent(ptvp->clk);
...
static s32 tv_clk_enable(struct disp_device *ptv)
{
    ...
    ret = disp_sys_clk_enable(ptvp->bus_clk);
    if (ret != 0)
...
static s32 tv_clk_disable(struct disp_device *ptv)
{
    ...
    disp_sys_clk_disable(ptvp->clk);

    if (!ptvp->bus_clk) {
        DE_WRN("bus_clk is NULL\n");
        return DIS_FAIL;
    }
    disp_sys_clk_disable(ptvp->bus_clk);
...
#else
    ptv->timings.dclk_rate_set = disp_sys_clk_get_rate(ptvp->clk);
#endif
...
s32 disp_tv_enable(struct disp_device *ptv)
{
    ...
    ret = disp_sys_register_irq(ptvp->irq_no, 0, disp_tv_event_proc,
                  (void *)ptv, 0, 0);
    if (ret != 0)
        DE_WRN("tv request irq failed!\n");

    disp_sys_enable_irq(ptvp->irq_no);
    flags = hal_spin_lock_irqsave(&g_tv_data_lock);
    ptvp->enabled = 1;
    hal_spin_unlock_irqrestore(&g_tv_data_lock, flags);
...
#if !defined(CONFIG_COMMON_CLK_ENABLE_SYNCBOOT)
    if (ptvp->clk &&
       (disp_clock_is_enabled(ptvp->clk) == 0))
        ret = 0;
#endif
...
s32 disp_tv_check_support_mode(struct disp_device *ptv, u32 tv_mode)
{
            ...
            unsigned int *gamma_table, unsigned int size)
{
    ...
    p_extend_info =
        hal_malloc(sizeof(struct panel_extend_para));
...
s32 disp_init_tv(void)
{
    ...
    g_ptv_devices = hal_malloc(sizeof(struct disp_device) * num_devices_support_tv);
    ...
    memset(g_ptv_devices, 0, sizeof(struct disp_device) * num_devices_support_tv);

    g_ptv_private = hal_malloc(num_devices_support_tv * sizeof(*p_tvp));
    ...
    memset(g_ptv_private, 0, num_devices_support_tv * sizeof(*p_tvp));
    ...
    for (hwdev_index = 0; hwdev_index < num_devices; hwdev_index++) {
        ...
        ret = hal_cfg_get_keyvalue(type_name, "status", (int32_t *)str, 4);
        ...
        ret = hal_cfg_get_keyvalue(type_name, "interface", &value, sizeof(value));
        ...
        p_tvp->clk = g_init_para.clk_tcon_lcd[hwdev_index];
        p_tvp->bus_clk = g_init_para.clk_bus_tcon_lcd[hwdev_index];
...
malloc_err:
    hal_free(g_ptv_devices);
    hal_free(g_ptv_private);
    g_ptv_devices = NULL;
    g_ptv_private = NULL;
