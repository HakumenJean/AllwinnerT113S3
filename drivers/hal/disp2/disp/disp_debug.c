...
static void printf_from_to(unsigned long from, unsigned long to)
{
    ...
    while(num--){
        hal_printf("0x%08lx: ",from);
        hal_printf("0x%08x 0x%08x 0x%08x 0x%08x\n",
...
int cmd_disp_debug(int argc, const char **argv)
{
    ...
    if (argc == 1) {
        ...
        while(i < argc) {
            ...
            if ( ! hal_strcmp(argv[i], "-r")) {
                print_reg();
            }
            /*switch display*/
            if ( ! hal_strcmp(argv[i], "-s")) {
                if (argc > i+3) {
                    i+=1;
