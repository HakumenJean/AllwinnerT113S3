...
static void sunxi_clear_fifo_status(unsigned long reg_base)
{
    ...
    reg_val  = SET_BITS(FIFO_OVERRUN_PENDING, FIFO_OVERRUN_PENDIN_WIDTH, reg_val, 0x0);
    reg_val  = SET_BITS(FIFO_DATA_PENDING, FIFO_DATA_PENDING_WIDTH, reg_val, 0x0);
...
{
    sunxi_set_acqiure_time(reg_base, 63);
    sunxi_set_frequency_divider(reg_base, 6);
    sunxi_set_clk_divider(reg_base, 2);
...
u32 sunxi_tpadc_ch_select(unsigned long reg_base, tp_channel_id id)
{
    u32 reg_val;
...
int sunxi_tpadc_ch_deselect(unsigned long reg_base, tp_channel_id id)
{
    u32 reg_val;
...
static void hal_tpadc_callback(void *param, uint32_t x_data, uint32_t y_data, data_flag_t flag)
{
    TPADC_INFO("tpadc : tpadc mode interrupt, data_flag is %ld, x data %d, y data %d", flag, x_data, y_data);
}

static void hal_tpadc_adc_callback(void *param, uint32_t data, tp_channel_id channel)
{
...
static void tpadc_handler(int irq, void *dev)
{
    ...
    if (reg_val & TP_DATAPEND) {
        ...
        tpadc->callback(tpadc->param, x_data, y_data, DATA_XY);
    }

    if (reg_val & TP_UPPEND) {
        tpadc->callback(tpadc->param, 0, 0, DATA_UP);
    }
...
hal_tpadc_status_t hal_tpadc_init(void)
{
    ...
    sunxi_set_up_debou_time(tpadc->reg_base, 5);

    sunxi_set_pressure_thresholed(tpadc->reg_base, 0xfff);
    sunxi_pressure_enable(tpadc->reg_base);
    sunxi_tp_mode_fifo_select(tpadc->reg_base, 0x00);
    sunxi_set_sensitivity(tpadc->reg_base, 8);
    ...
    writel(0xC00, tpadc->reg_base + TP_CDAT);

    reg_val = readl(tpadc->reg_base + TP_CTRL1);
    reg_val |= (1 << 9) | (1 << 7);
    writel(reg_val, tpadc->reg_base + TP_CTRL1);
...
static void tpadc_adc_handler(int irq, void *dev)
{
    hal_tpadc_t *tpadc = (hal_tpadc_t *)dev;
    u32 reg_val;
    u32 data;
    ...
    if (reg_val & TP_DATAPEND) {
        ...
        if (reg_val & TP_CH0_SELECT)
        {
            ...
            if (tpadc->adc_callback[0])
                tpadc->adc_callback[0](tpadc->adc_param[0], data, TP_CH0_SELECT);
        }
        ...
        if (reg_val & TP_CH1_SELECT)
        {
            ...
            if (tpadc->adc_callback[1])
                tpadc->adc_callback[1](tpadc->adc_param[1], data, TP_CH1_SELECT);
        }
        ...
        if (reg_val & TP_CH2_SELECT)
        {
            ...
            if (tpadc->adc_callback[2])
                tpadc->adc_callback[2](tpadc->adc_param[2], data, TP_CH2_SELECT);
        }
        ...
        if (reg_val & TP_CH3_SELECT)
        {
            ...
            if (tpadc->adc_callback[3])
                tpadc->adc_callback[3](tpadc->adc_param[3], data, TP_CH3_SELECT);
        }
...
hal_tpadc_status_t hal_tpadc_adc_register_callback(tp_channel_id channel, tpadc_adc_usercallback_t user_adc_callback, void *adc_param)
{
    hal_tpadc_t *tpadc = &hal_tpadc;
    ...
    if (user_adc_callback == NULL) {
        ...
        tpadc->adc_param[channel] = adc_param;
    }
...
hal_tpadc_status_t hal_tpadc_adc_init(void)
{
    ...
    if (hal_request_irq(tpadc->irq_num, tpadc_adc_handler, "tpadc adc", tpadc))
    {
        TPADC_ERR("tpadc request irq(%d) failed \n", tpadc->irq_num);
        return TPADC_ERROR;
    }
    ...
    TPADC_INFO("tpadc adc init success");
