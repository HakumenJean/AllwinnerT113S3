...
//		do_div(temp, out_h);
	...
	hal_memcpy(&p_reg->vs_y_ch_hor_filter_coef[0],
	...
	case G2D_FORMAT_Y8:
		...
		hal_memcpy(&p_reg->vs_y_ch_ver_filter_coef[0],
		       &linearcoefftab32[0],
	...
	case G2D_FORMAT_IYUV422_Y1U0Y0V0:{
			...
			hal_memcpy(&p_reg->vs_c_ch_hor_filter_coef[0],
...
static __s32 scal_destory(struct scaler_submodule *p_scal)
{
	...
	if (p_scal) {
	    hal_free(p_scal->reg_blks);
		p_scal->reg_blks = NULL;

		hal_free(p_scal->reg_info);
...
struct scaler_submodule *g2d_scaler_submodule_setup(struct g2d_mixer_frame *p_frame)
{
	...
	hal_memset(p_scal, 0, sizeof(struct scaler_submodule));
	...
	hal_memset(p_scal->reg_blks, 0, sizeof(struct g2d_reg_block) * p_scal->reg_blk_num);
	hal_memset(p_scal->reg_info, 0, sizeof(struct g2d_reg_mem_info));
