...
static __s32 ovl_u_destory(struct ovl_u_submodule *p_ovl_u)
{
	...
	if (p_ovl_u) {
	    hal_free(p_ovl_u->reg_blks);
		p_ovl_u->reg_blks = NULL;

		hal_free(p_ovl_u->reg_info);
...
g2d_ovl_u_submodule_setup(struct g2d_mixer_frame *p_frame)
{
	...
	hal_memset(p_ovl_u, 0, sizeof(struct ovl_u_submodule));
	...
	hal_memset(p_ovl_u->reg_blks, 0, sizeof(struct g2d_reg_block) * p_ovl_u->reg_blk_num);
	hal_memset(p_ovl_u->reg_info, 0, sizeof(struct g2d_reg_mem_info));
