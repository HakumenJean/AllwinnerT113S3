...
{
	if (p_ovl_v) {
	    hal_free(p_ovl_v->reg_blks);
		p_ovl_v->reg_blks = NULL;

		hal_free(p_ovl_v->reg_info);
...
g2d_ovl_v_submodule_setup(struct g2d_mixer_frame *p_frame)
{
	...
	hal_memset(p_ovl_v, 0, sizeof(struct ovl_v_submodule));
	...
	hal_memset(p_ovl_v->reg_blks, 0, sizeof(struct g2d_reg_block) * p_ovl_v->reg_blk_num);
	hal_memset(p_ovl_v->reg_info, 0, sizeof(struct g2d_reg_mem_info));
