...
__s32 bld_csc_reg_set(struct blender_submodule *p_bld, __u32 csc_no, g2d_csc_sel csc_sel)
{
	...
	case G2D_RGB2YUV_709:
		hal_memcpy(csc_base_addr, rgb2Ycbcr_709, 12 * sizeof(unsigned int));
		break;
	case G2D_YUV2RGB_709:
	    hal_memcpy(csc_base_addr, Ycbcr2rgb_709, 12 * sizeof(unsigned int));
		break;
	case G2D_RGB2YUV_601:
	    hal_memcpy(csc_base_addr, rgb2Ycbcr_601, 12 * sizeof(unsigned int));
		break;
	case G2D_YUV2RGB_601:
	    hal_memcpy(csc_base_addr, Ycbcr2rgb_601, 12 * sizeof(unsigned int));
		break;
	default:
...
static __s32 bld_destory(struct blender_submodule *p_bld)
{
	...
	if (p_bld) {
	    hal_free(p_bld->reg_blks);
		p_bld->reg_blks = NULL;

		hal_free(p_bld->reg_info);
		p_bld->reg_info = NULL;
		hal_free(p_bld);
		ret = 0;
	}
...
g2d_bld_submodule_setup(struct g2d_mixer_frame *p_frame)
{
	...
	hal_memset(p_bld, 0, sizeof(struct blender_submodule));
	...
	hal_memset(p_bld->reg_blks, 0, sizeof(struct g2d_reg_block) * p_bld->reg_blk_num);
	hal_memset(p_bld->reg_info, 0, sizeof(struct g2d_reg_mem_info));
	return p_bld;
FREE_WB:
    hal_free(p_bld->reg_blks);
    hal_free(p_bld->reg_info);
