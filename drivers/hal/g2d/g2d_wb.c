...
static __s32 wb_destory(struct wb_submodule *p_wb)
{
	...
	if (p_wb) {
	    hal_free(p_wb->reg_blks);
		p_wb->reg_blks = NULL;

		hal_free(p_wb->reg_info);
		p_wb->reg_info = NULL;
		ret = 0;
		hal_free(p_wb);
	}
...
struct wb_submodule *g2d_wb_submodule_setup(struct g2d_mixer_frame *p_frame)
{
	...
	hal_memset(p_wb, 0, sizeof(struct wb_submodule));
	...
	hal_memset(p_wb->reg_blks, 0, sizeof(struct g2d_reg_block) * p_wb->reg_blk_num);
	hal_memset(p_wb->reg_info, 0, sizeof(struct g2d_reg_mem_info));
