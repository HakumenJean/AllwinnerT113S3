				...
				__u32 flag, g2d_ck *ck_para)
{
	...
	rect0.x = src->coor.x;
	rect0.y = src->coor.y;
	rect0.w = src->clip_rect.w;/* overlay size */
	rect0.h = src->clip_rect.h;
				 ...
				 struct mixer_para *p_para)
{
	...
	hal_memset(p_task->p_rcq_info->reg_blk, 0, sizeof(*(p_task->p_rcq_info->reg_blk)) *
			p_task->p_rcq_info->cur_num);
...
FREE:
    hal_free(p_frame->wb);
    hal_free(p_frame->ovl_v);
    hal_free(p_frame->ovl_u);
    hal_free(p_frame->bld);
    hal_free(p_frame->scal);
OUT:
	return ret;
}
			   ...
			   struct mixer_para *p_para)
{
	...
	if ((p_para->op_flag == OP_BITBLT) && (p_task->p_g2d_info->scan_order == G2D_SM_DTLR))
		g2d_mixer_scan_order_fun(G2D_SM_DTLR_1 >> 28);
	hal_dcache_clean(p_task->p_rcq_info->vir_addr,p_task->p_rcq_info->rcq_reg_mem_size);
	isb();
	...
	hal_memcpy(p_task->p_para, p_para,
...
static __s32 g2d_mixer_task_destory(struct g2d_mixer_task *p_task)
{
	...
	if (p_task->p_rcq_info) {
	    hal_free(p_task->p_rcq_info->reg_blk);
	    hal_free(p_task->p_rcq_info);
	}

	if (p_task->p_para)
	    hal_free(p_task->p_para);

	g2d_list_del(p_task);

	id_free(idr, p_task->task_id);
	hal_free(p_task->frame);
	hal_free(p_task);
			 ...
			 unsigned int frame_len)
{
	...
	hal_memset(task, 0, sizeof(struct g2d_mixer_task));
	...
	hal_memset(task->frame, 0, frame_len*sizeof(struct g2d_mixer_frame));
	hal_memset(task->p_rcq_info, 0, sizeof(struct g2d_rcq_mem_info));
	...
	hal_memcpy(task->p_para, p_para, sizeof(struct mixer_para) * task->frame_cnt);
