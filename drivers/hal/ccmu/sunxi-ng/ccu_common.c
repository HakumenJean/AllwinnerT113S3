...
int ccu_common_init(unsigned long reg, const struct sunxi_ccu_desc *desc)
{
    ...
    for (i = 0; i < desc->num_ccu_clks; i++)
    {
        ...
        cclk->lock = (hal_spinlock_t)HAL_SPINLOCK_INIT;
        hal_spin_lock_init(&cclk->lock);
        cclk->base = reg;
    }
    ...
    for (i = 0; i < desc->hw_clks->num; i++)
    {
        ...
        if (ret)
        {
            hal_printf("Couldn't register clock %d - %s\n", i, name);
            goto err_clk_unreg;
        }

    }

    reset = (struct ccu_reset *)hal_malloc(sizeof(*reset));
    if (!reset)
    {
    ...
    reset->lock = (hal_spinlock_t)HAL_SPINLOCK_INIT;
    hal_spin_lock_init(&reset->lock);
    reset->base = reg;
