...
{
    return readl(CE_BASE_REG + offset);
}

static void ce_writel(uint32_t offset, uint32_t val)
{
    writel(val, CE_BASE_REG + offset);
}
...
{
    hal_memcpy(dst, src, size);
}


void hal_ce_iv_set(unsigned char *iv, int size, ce_task_desc_t *task)
{
...
void hal_ce_cnt_set(unsigned char *cnt, int size, ce_task_desc_t *task)
{
