...
#include "hal_efuse.h"
#include "platform/efuse_sun8iw20.h"
#include "efuse.h"
...
#define EFUSE_OEM1_NAME             "oem1"
#define EFUSE_OEM2_NAME             "oem2"
#define EFUSE_OEM3_NAME             "oem3"
...
#ifdef EFUSE_THERMAL_SENSOR
EFUSE_DEF_ITEM(EFUSE_THM_SENSOR_NAME, EFUSE_THERMAL_SENSOR, 64, -1, SCC_THERMAL_SENSOR_BURNED_FLAG, EFUSE_RO),
#endif
#ifdef EFUSE_FT_ZONE
EFUSE_DEF_ITEM(EFUSE_FT_ZONE_NAME, EFUSE_FT_ZONE, 128, -1, SCC_FT_ZONE_BURNED_FLAG, EFUSE_RO),
#endif
#ifdef EFUSE_ROTPK
EFUSE_DEF_ITEM(EFUSE_ROTPK_NAME, EFUSE_ROTPK, 256, -1, SCC_ROTPK_BURNED_FLAG, EFUSE_RO),
#endif
#ifdef EFUSE_NV1
EFUSE_DEF_ITEM(EFUSE_NV1_NAME, EFUSE_NV1, 32, -1, SCC_NV1_BURNED_FLAG, EFUSE_RO),
#endif
#ifdef EFUSE_PSENSOR
EFUSE_DEF_ITEM("psensor", EFUSE_PSENSOR, 32, -1, SCC_PSENSOR_BURNED_FLAG, EFUSE_RO),
#endif
#ifdef EFUSE_TVOUT
EFUSE_DEF_ITEM(EFUSE_TV_OUT_NAME, EFUSE_TVOUT, SID_TVOUT_SIZE, -1, -1, EFUSE_RO),
#endif
#ifdef EFUSE_SSK
EFUSE_DEF_ITEM(EFUSE_SSK_NAME, EFUSE_SSK, 128, -1, SCC_SSK_BURNED_FLAG, EFUSE_RO),
#endif
#ifdef EFUSE_OEM1
EFUSE_DEF_ITEM(EFUSE_OEM1_NAME, EFUSE_OEM1, 128, -1, SCC_OEM1_BURNED_FLAG, EFUSE_RO),
#endif
#ifdef EFUSE_OEM2
EFUSE_DEF_ITEM(EFUSE_OEM2_NAME, EFUSE_OEM2, 128, -1, SCC_OEM2_BURNED_FLAG, EFUSE_RO),
#endif
#ifdef EFUSE_OEM3
EFUSE_DEF_ITEM(EFUSE_OEM3_NAME, EFUSE_OEM3, 384, -1, SCC_OEM3_BURNED_FLAG, EFUSE_RO),
#endif
...
static uint _efuse_reg_read_key(uint key_index)
{
    ...
    hal_writel(reg_val, (unsigned long)SID_PRCTL);
       ...
       // do not need isb
       // actually need delay 5 nop instructions
       isb();
#endif
...
{
#ifdef CONFIG_ARCH_SUN20IW2
    hal_readl((unsigned long)SUNXI_SID_BASE + 0x84);
#endif
...
efuse_key_map_new_t *efuse_search_key_by_name(const char *key_name)
{
    ...
    for (; key_map->size != 0; key_map++) {
        if (hal_strlen(key_map->name) != hal_strlen(key_name)) {
            continue;
        }
        if (!hal_memcmp(key_name, key_map->name, hal_strlen(key_map->name))) {
            break;
        }
    }
    return key_map;
...
#ifdef SUNXI_SYSCFG_VER
uint efuse_read_chip_ver(void)
{
    uint val;

    val = hal_readl((unsigned long)SUNXI_SYSCFG_VER) & VER_MARK;
