...
#define EFUSE_DBG(fmt,args...)  hal_printf(fmt ,##args)
...
#endif
#define EFUSE_ERR(fmt,args...)  hal_printf(fmt ,##args)
...
int hal_efuse_write(char *key_name, unsigned char *key_data, size_t key_bit_len)
{
    ...
    if ((hal_strlen(key_name) == 0)|| (key_len == 0) || ((unsigned long)key_data%4 != 0))
    ...
    if (hal_strcmp(key_name, "rotpk") == 0) {
    }
...
int hal_efuse_read(char *key_name, unsigned char *key_data, size_t key_bit_len)
{
    ...
    for(i = 0;i<k_u32_l;i++)
    {
        ...
        {
            hal_memcpy((void*)(u8_p + i * 4),(void*)(&tmp),4);
        }
        offset += 4;
        tmp_sz += 4;
    }
    ...
    if(bit_lft)
    {
        ...
        tmp = efuse_sram_read_key(offset);
        hal_memcpy((void*)(u8_p + k_u32_l * 4),(void*)(&tmp),
...
int hal_efuse_write_ext(uint32_t start_bit, uint32_t bit_num, uint8_t *data)
{
    ...
    hal_memset(sid_data, 0, word_cnt * 4);
    hal_memcpy(sid_data, data, word_size);
