...
#ifdef LEDC_DEBUG
#define ledc_info(fmt, args...)  hal_printf("%s()%d - "fmt, __func__, __LINE__, ##args)
#else
...
static void ledc_set_output_mode(const char *str)
{
    ...
    unsigned int reg_val ;
    if (str != NULL) {
        if (!hal_strncmp(str, "GRB", 3))
            val = LEDC_OUTPUT_GRB;
        else if (!hal_strncmp(str, "GBR", 3))
            val = LEDC_OUTPUT_GBR;
        else if (!hal_strncmp(str, "RGB", 3))
            val = LEDC_OUTPUT_RGB;
        else if (!hal_strncmp(str, "RBG", 3))
            val = LEDC_OUTPUT_RBG;
        else if (!hal_strncmp(str, "BGR", 3))
            val = LEDC_OUTPUT_BGR;
        else if (!hal_strncmp(str, "BRG", 3))
            val = LEDC_OUTPUT_BRG;
        else
            return;
    } else {
...
void hal_ledc_trans_data(struct ledc_config *ledc)
{
    ...
    if (ledc->length <= SUNXI_LEDC_FIFO_DEPTH) {
        ...
        hal_dcache_clean(ledc->data, sizeof(ledc->data));
