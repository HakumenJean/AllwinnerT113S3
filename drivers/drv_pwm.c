...
struct pwm_gpio_t user_pwm_gpio[PWM_NUM] = {
    {   // 0
        .pwm_pin = GPIO_PD16,
        .pwm_function = 5,
    },
...
void hal_pwm_pinctrl_init(hal_pwm_t sunxi_pwm, int channel)
{
    hal_gpio_pinmux_set_function(user_pwm_gpio[channel].pwm_pin, user_pwm_gpio[channel].pwm_function);
}

void hal_pwm_pinctrl_exit(hal_pwm_t sunxi_pwm, uint32_t channel)
{
    hal_gpio_pinmux_set_function(user_pwm_gpio[channel].pwm_pin, 0);
}
...
int rt_hw_pwm_init(void)
{
    ...
    rt_device_pwm_register(&_hal_pwm_device.parent, "pwm", &pwm_ops, RT_NULL);

    LOG_I("pwm init success");

    return RT_EOK;
}
INIT_PREV_EXPORT(rt_hw_pwm_init);
